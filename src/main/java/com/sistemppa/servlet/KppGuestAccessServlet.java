...
public class KppGuestAccessServlet extends HttpServlet {
    ...
    private void handleGenerateLink(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String appBaseUrl = resolvePublicAppBaseUrl(request);
    ...
    private String resolvePublicAppBaseUrl(HttpServletRequest request) {
        String configuredBaseUrl = normalizeBaseUrl(str(getServletContext().getInitParameter("app.base.url")));
        ...
        // If configured base URL is localhost but current request comes from a non-local host,
        // prefer request host so recipients can open the generated link.
        if (isLocalHostUrl(configuredBaseUrl) && !isLocalHostName(str(request.getServerName()))) {
            return requestBaseUrl;
        }
    ...
    private String buildRequestBaseUrl(HttpServletRequest request) {
        return normalizeBaseUrl(request.getScheme() + "://" + request.getServerName()
                + (request.getServerPort() == 80 || request.getServerPort() == 443 ? "" : ":" + request.getServerPort())
                + request.getContextPath());
    }

    private String normalizeBaseUrl(String baseUrl) {
        String value = str(baseUrl);
    ...
    private boolean isLocalHostUrl(String url) {
        String value = str(url).toLowerCase(Locale.ROOT);
    ...
    private boolean isLocalHostName(String hostName) {
        String value = str(hostName).toLowerCase(Locale.ROOT);
